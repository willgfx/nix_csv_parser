        ...
        if len(sys.argv) <= 1:
            ...
            for i, arg in enumerate(args):
                if (arg[0] == '-' and arg[-1] == 'w') or arg == '--wait':
                    ...
                    if len(args) > i+1 and args[i+1].isdigit():
                        ...
                        break
                    # Next arg isn't a value as expected, error out
                    else:
                        msg_display(
            ...
            if 'c' in options or 'clipboard' in options:
                ...
                if '.csv' in clipboard_data.lower():
                    self.file = clipboard_data.strip('"')  # Strip " because: Windows
    ...
    def get_swatches(self):
        ...
        if self.mode == 'clipboard':
            ...
            if 'HEX,' in clipboard_data:  # simple check for valid CSV data
                ...
                msg_display(
                    ['error_clipboard_nodata', 'info_tryhelp'], exit_after=True,
                    wait=self.wait
                )
        else:
            if self.file is not None:
                ...
                msg_display(
                    ['error_nodata', 'info_usage'], exit_after=True,
                    wait=self.wait
                )
